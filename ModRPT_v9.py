...
if uploaded_file is not None:
    try:
        ...
        # ==============================================
        # WEDGE MODELING SECTION - MOVED TO BE INDEPENDENT
        # ==============================================
        if show_wedge_model and model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            # Define middle interface depth range from the selected depth interval
            middle_top = ztop + (zbot - ztop) * 0.4
            middle_bot = ztop + (zbot - ztop) * 0.6
            
            # Use rock physics results as default values from the mixed case
            ...
            shale_vp = logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top), 'VP'].values.mean() if len(logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top)]) > 0 else logs.VP.mean()
            shale_vs = logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top), 'VS'].values.mean() if len(logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top)]) > 0 else logs.VS.mean()
            shale_rho = logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top), 'RHO'].values.mean() if len(logs.loc[(logs.DEPTH >= middle_top - (middle_bot-middle_top)) & (logs.DEPTH < middle_top)]) > 0 else logs.RHO.mean()
        ...
        if crossplot:
            try:
                ...
                st.warning(f"Interactive crossplot failed: {str(e)}")
                # Fallback static crossplot
                st.subheader("Static Crossplot (All Fluid Cases)")
                fig_static, ax_static = plt.subplots(figsize=(10, 8))
                
                colors_map = {'Brine': 'blue', 'Oil': 'green', 'Gas': 'red', 'Mixed': 'magenta', 'Shale': 'gray'}
                for case, color in colors_map.items():
                    if case == 'Brine':
                        mask = logs['LFC_B'] == 1
                        ip_col = 'IP_FRMB'
                        vpvs_col = 'VPVS_FRMB'
                    ...
                    if mask.any() and ip_col in logs.columns and vpvs_col in logs.columns:
                        ax_static.scatter(logs.loc[mask, ip_col], logs.loc[mask, vpvs_col], 
                                        c=color, label=case, alpha=0.5, s=10)
                
                ax_static.set_xlabel('IP (m/s*g/cc)')
                ax_static.set_ylabel('Vp/Vs')
                ax_static.set_title('IP vs Vp/Vs Crossplot (All Fluid Cases)')
                ax_static.legend()
                ax_static.grid(True)
                st.pyplot(fig_static)
        else:
            st.warning("Could not generate interactive crossplot - showing static crossplot instead")
            st.subheader("Static Crossplot (All Fluid Cases)")
            fig_static, ax_static = plt.subplots(figsize=(10, 8))
        ...
        ax2[0].scatter(logs.IP_original if 'IP_original' in logs.columns else logs.IP, 
                      logs.VPVS_original if 'VPVS_original' in logs.columns else logs.VPVS, 
                      20, logs[cluster_col] if cluster_col in logs.columns else 0, 
                      marker='o', edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        ax2[0].set_xlabel('IP (m/s*g/cc)')
        ax2[0].set_ylabel('Vp/Vs(unitless)')
        
        ax2[1].scatter(logs.IP_FRMB, logs.VPVS_FRMB, 20, logs.LFC_B, marker='o', 
                      edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        ax2[1].set_xlabel('IP (m/s*g/cc)')
        ax2[1].set_ylabel('Vp/Vs(unitless)')
        
        if 'IP_FRMO' in logs.columns:
            ax2[2].scatter(logs.IP_FRMO, logs.VPVS_FRMO, 20, logs.LFC_O, marker='o', 
                          edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        else:
            ax2[2].scatter(logs.IP_FRMB, logs.VPVS_FRMB, 20, logs.LFC_B, marker='o', 
                          edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        ax2[2].set_xlabel('IP (m/s*g/cc)')
        ax2[2].set_ylabel('Vp/Vs(unitless)')
        
        ax2[3].scatter(logs.IP_FRMG, logs.VPVS_FRMG, 20, logs.LFC_G, marker='o', 
                      edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        ax2[3].set_xlabel('IP (m/s*g/cc)')
        ax2[3].set_ylabel('Vp/Vs(unitless)')
        
        ax2[4].scatter(logs.IP_FRMMIX, logs.VPVS_FRMMIX, 20, logs.LFC_MIX, marker='o', 
                      edgecolors='none', alpha=0.5, cmap=cmap_facies, vmin=0, vmax=5)
        ...
        # Continue with remaining modules (Sonic Log Prediction, Inversion Feasibility, 3D Crossplot, Histograms, AVO Modeling, etc.)
        ...
        if predict_sonic and 'PHI' in logs.columns and 'VSH' in logs.columns:
            ...
            if st.button("Train Prediction Models", key="train_sonic") and features:
        ...
        if inversion_feasibility:
            ...
            with st.spinner("Analyzing inversion feasibility..."):
                ...
                if feasibility:
                    ...
                    if feasibility['correlation_matrix'] is not None:
                    ...
                    if feasibility['sensitivity']:
                        st.subheader("Sensitivity Analysis")
                        sens_df = pd.DataFrame.from_dict(feasibility['sensitivity'], 
                                                        orient='index', columns=['Correlation'])
                        st.dataframe(sens_df.style.format("{:.2f}").background_gradient(cmap='coolwarm', vmin=-1, vmax=1))
        ...
        if show_3d_crossplot and model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            for case, color, case_label in [('B', 'blue', 'Brine'), ('O', 'green', 'Oil'), 
                                            ('G', 'red', 'Gas'), ('MIX', 'magenta', 'Mixed')]:
                lfc_col = f'LFC_{case}'
                if lfc_col in logs.columns:
                    mask = logs[lfc_col] == (1 if case == 'B' else 2 if case == 'O' else 3 if case == 'G' else 4)
                    ip_col = f'IP_FRM{case}'
                    vpvs_col = f'VPVS_FRM{case}'
                    rho_col = f'RHO_FRM{case}'
                    
                    if mask.any() and ip_col in logs.columns:
                        ax3d.scatter(
                            logs.loc[mask, ip_col],
                            logs.loc[mask, vpvs_col],
                            logs.loc[mask, rho_col],
                            c=color, label=case_label, alpha=0.5, s=10
                        )
        ...
        if show_histograms and model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            lfc_col = 'LFC_MIX' if 'LFC_MIX' in logs.columns else 'LFC_B'
            ax_hist[1,1].hist(logs[lfc_col], bins=[0,1,2,3,4,5,6], alpha=0.5, rwidth=0.8, align='left')
