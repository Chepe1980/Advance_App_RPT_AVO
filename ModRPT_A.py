...
# ==============================================
# Wedge Modeling Functions (Corrected)
# ==============================================
...
def create_interactive_crossplot(logs, depth_range=None):
    ...
    try:
        ...
        # Filter data based on depth range if provided
        if depth_range is not None:
            plot_data = logs[(logs['DEPTH'] >= depth_range[0]) & (logs['DEPTH'] <= depth_range[1])]
        else:
            plot_data = logs.copy()
            
        # Filter out NaN values and ensure numeric data
        plot_data = plot_data[['IP', 'VPVS', 'LFC_Label', 'DEPTH']].dropna()
...
if uploaded_file is not None:
    try:
        ...
        if show_histograms and model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            ax_hist[0,0].hist(filtered_logs.IP_FRMB, bins=30, alpha=0.5, label='Brine', color='blue')
            ax_hist[0,0].hist(filtered_logs.IP_FRMO, bins=30, alpha=0.5, label='Oil', color='green')
            ax_hist[0,0].hist(filtered_logs.IP_FRMG, bins=30, alpha=0.5, label='Gas', color='red')
            ax_hist[0,0].hist(filtered_logs.IP_FRMMIX, bins=30, alpha=0.5, label='Mixed', color='magenta')
            ax_hist[0,0].set_xlabel('IP (m/s*g/cc)')
            ax_hist[0,0].set_ylabel('Frequency')
            ax_hist[0,0].legend()
            
            ax_hist[0,1].hist(filtered_logs.VPVS_FRMB, bins=30, alpha=0.5, label='Brine', color='blue')
            ax_hist[0,1].hist(filtered_logs.VPVS_FRMO, bins=30, alpha=0.5, label='Oil', color='green')
            ax_hist[0,1].hist(filtered_logs.VPVS_FRMG, bins=30, alpha=0.5, label='Gas', color='red')
            ax_hist[0,1].hist(filtered_logs.VPVS_FRMMIX, bins=30, alpha=0.5, label='Mixed', color='magenta')
            ax_hist[0,1].set_xlabel('Vp/Vs')
            ax_hist[0,1].legend()
            
            ax_hist[1,0].hist(filtered_logs.RHO_FRMB, bins=30, color='blue', alpha=0.7)
            ax_hist[1,0].hist(filtered_logs.RHO_FRMO, bins=30, color='green', alpha=0.7)
            ax_hist[1,0].hist(filtered_logs.RHO_FRMG, bins=30, color='red', alpha=0.7)
            ax_hist[1,0].hist(filtered_logs.RHO_FRMMIX, bins=30, color='magenta', alpha=0.7)
        ...
        if model_choice not in ["Soft Sand RPT (rockphypy)", "Stiff Sand RPT (rockphypy)"]:
            ...
            # Plot frequency domain analysis (Fixed)
            ...
            for idx, case in enumerate(cases):
                ...
                for i, t in enumerate(t_samp_filtered):
                    ...
                    if time_idx >= syn_gather.shape[1]:
                        continue
                ...
                if len(freqs) > 0 and len(t_samp_filtered) > 0:
            ...
            try:
                ...
                for col_idx, case in enumerate(cases):
                    ...
                    if mid_angle_idx < syn_gather.shape[0]:
                        ...
                        ax_cwt[1, col_idx].set_ylim(np.min(time_series)*1.1, np.max(time_series)*1.1)
